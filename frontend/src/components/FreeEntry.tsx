...
export default function FreeEntry({ onSwitchToGuided, onSwitchToSimple }: FreeEntryProps) {
  ...
  const saveEntryEdit = async (entryId: string, mode: string) => {
    try {
      ...
      console.log('Starting saveEntryEdit:', { entryId, mode, editingContent });
      
      if (mode === 'casual' || mode === 'free_entry') {
        const numericId = parseInt(entryId.replace(`${mode}_`, ''));
        const token = localStorage.getItem('access_token');
        
        console.log('Parsed details:', { numericId, token: token ? 'exists' : 'missing' });
        
        // Use general diary endpoint for all modes (works for both casual and free_entry)
        const requestUrl = `http://localhost:8001/diary/entry/${numericId}`;
        const requestBody = { content: editingContent };
        
        console.log('Making API request:', { requestUrl, requestBody, mode });
      ...
      alert(language === 'en' ? 'Changes saved successfully!' : '更改已成功保存！');
      
    } catch (error) {
      console.error('Error saving entry edit:', error);
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
      const displayMessage = language === 'en' 
