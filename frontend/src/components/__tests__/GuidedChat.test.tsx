...
describe('GuidedChat Component', () => {
  ...
  test('shows AI model selection dropdown', async () => {
    render(
      <TestWrapper>
        <GuidedChat onSwitchToLegacy={mockOnSwitchToLegacy} />
      </TestWrapper>
  ...
  test('shows Casual Mode button', async () => {
    ...
    const casualModeButton = screen.getByText('Casual Mode');
  ...
  test('starts new session when Start Fresh is clicked', async () => {
    ...
    await waitFor(() => {
      expect(mockedGuidedDiaryAPI.startSession).toHaveBeenCalledWith('en', 'gemma3:4b');
    });
  });
